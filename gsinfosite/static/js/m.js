...
function treeSelectHandler(node) {
    ...
    var subBoxNumber = node.text.substring(index + 1, index1).split('-')[1] ? node.text.substring(index + 1, index1).split('-')[1] : ''
...
function operationFormatter(value, row, index) {
    if (row.boxNumber.indexOf("-")>=0) {
        var boxNumber = row.boxNumber.split('-')[0]
        var subBoxNumber = row.boxNumber.split('-')[1] ? row.boxNumber.split('-')[1] : '';
    }
    if(row.boxNumber.indexOf("-")< 0){
    ...
    if (!value) {
        // 外观信息采集, 频谱分析, 测量称重三环节均已完成, 可以进行复核
        var operator = $('#operator').val();
        //href="exploreThing/' + boxNumber + '/' + row.serialNumber + '?isVerify=1&operator=' + operator + '&subBoxNumber=' + subBoxNumber + '"+
        return '<div style="float:center" onclick="informationDialog()" ><a  target="blank"  style="text-decoration:none;color:blue;">复核</a></div>';
    }
    else {
        return '-';
    }
...
function informationDialog(){
     ...
     $('#createWorkDlg').dialog('open').dialog('center').dialog('setTitle', '更新信息')
}
var url;
function newWork() {
...
function updateDate(date) {
    ...
    $.ajax({
        url: 'getDurationCompleteThingAmount/',
